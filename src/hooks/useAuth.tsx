...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    ...
    // Safety net: never strand the user on a spinner beyond 3 s
    const safetyTimer = setTimeout(resolve, 3000)

    // 1. One-time session hydration — resolves loading as fast as possible
    ...
    // 2. Live listener — keeps state in sync after initial load
    const { data: { subscription } } = supabase.auth.onAuthStateChange(
      async (_event, sess) => {
        ...
        // Only resolve here if getSession hasn't already
        resolve()
      }
    )
